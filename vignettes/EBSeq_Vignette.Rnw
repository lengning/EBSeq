...
check \verb+str(Data)+ and make sure it is a matrix instead of data frame. You may need to play around with the \verb+row.names+ 
...
\indent You may on an earlier version of EBSeq. The GetDEResults function
...
And you may check your package version by typing \verb+packageVersion("EBSeq")+.
